...
class Color:
	...
	# Đây gọi là annotation (@). Static method là phương thức được sử dụng mà không cần tạo object.
	# Cứ bỏ qua cái này
	@staticmethod
	def colors():
...
# Class mảnh (piece) có các chữ khác nhau
class Piece:
...
class Board:
	...
	def clear_tile(self, x, y):
		...
		# Di chuyển toàn bộ các ô xuống dưới một ô
	...
	def piece_can_move(self, x_move, y_move):
		"""Trả về true nếu piece có thể di chuyển, không thì false"""
	...
	def drop_piece(self):
		"""Hoặc là bỏ 1 piece dưới đáy, hoặc xóa một hàng đầy và tạo 1 piece mới."""
	...
	def piece_can_rotate(self, clockwise):
		"""Kiểm tra xem piece có xoay được không"""
	...
	def generate_piece(self):
		"""Tạo mới 1 piece và cho nó xuất hiện trên đỉnh màn hình."""
		if self.game_over:
			return
		...
		# Nếu mới tạo piece mà đã khogn6 di chuyển được thì nghĩa là thua
		if not self.piece_can_move(0, 0):
			# Hiện piece lên board
			self.finalize_piece()
